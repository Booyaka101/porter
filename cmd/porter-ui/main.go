...
func main() {
	...
	web.SetPortableMode(*portableMode)
	if *portableMode {
	...
	if *migrateFromMySQL || os.Getenv("MIGRATE_FROM_MYSQL") == "true" {
		...
		mysqlConfig := web.LoadDBConfigFromEnv()
		sqlitePath := web.GetSQLitePath()
		if err := web.MigrateFromMySQLToSQLite(mysqlConfig, sqlitePath); err != nil {
	...
	if *useMySQL || useMySQLEnv {
		...
		dbConfig := web.LoadDBConfigFromEnv()
		if err := web.InitMySQLDatabase(dbConfig); err != nil {
			log.Fatalf("Failed to initialize MySQL database: %v", err)
		}
		defer web.GetDB().Close()
		...
		dbPath := web.GetSQLitePath()
		if err := web.InitSQLiteDatabase(dbPath); err != nil {
	...
	server := &ooo.Server{
		...
		OnClose: func() {
			...
			web.StopScheduler()
			web.StopHealthPoller()
		},
		Silence: false,
	}

	// Setup API routes
	web.SetupRoutes(server.Router)
